    ...
    <header className="sticky top-0 z-50 border-b border-earth-200/70 bg-earth-50/92 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-5 sm:py-4 lg:px-8">
        <Link href="/" className="flex min-w-0 items-center gap-3" aria-label="The Dog Bond home">
          <Image src="/mark.svg" alt="The Dog Bond mark" width={40} height={40} className="h-10 w-10 shrink-0 rounded-xl sm:h-11 sm:w-11 sm:rounded-2xl" priority />
          <div className="min-w-0">
            <p className="font-display text-base font-bold tracking-tight text-earth-900 sm:text-xl">{site.name}</p>
      ...
      <nav className="mx-auto flex max-w-7xl gap-2 overflow-x-auto border-t border-earth-200/70 px-4 py-2.5 sm:gap-3 sm:px-5 sm:py-3 lg:hidden" aria-label="Mobile navigation">
        {navItems.map((item) => (
          <a key={item.href} href={item.href} className="shrink-0 rounded-full border border-earth-200 bg-white/70 px-3.5 py-2 text-xs font-bold text-earth-700 sm:px-4">
            {item.label}
          </a>
        ))}
      </nav>
